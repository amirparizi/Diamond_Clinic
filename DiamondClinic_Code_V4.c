...
int main()
{
   ...
   // test for send elevator and elevator_avail
		...
		//for (ii=0;ii<10000;ii++)
		{
			...
			{	
				...
				{	
					...
					for (i = 0; i < NUM_ELEVATORS; i++) 
					    {if (elevators[i].next_floor>=people[next_event.entity_type][next_event.entity_index].to_floor) //hp: door_time+epsilon
                        ...
                        elevators[i].time_to_reach = elevators[i].elevator_clock + elevators[i].elevator_time[1] + elevators[i].elevator_time[0] + travel_time - next_event.time;//hP we chinged this from - to +
					...
					{wait_time = min(wait_time,elevators[k].time_to_reach); //hp: let us check this
					}
					for (int k=0;k<NUM_ELEVATORS;k++)
					{ if (wait_time == elevators[k].time_to_reach)
						...
						if (elevators[k].next_floor < people[next_event.entity_type][next_event.entity_index].to_floor)
							{elevators[k].elevator_going_to = people[next_event.entity_type][next_event.entity_index].to_floor;
                            if (elevators[k].idle==1)
                               {elevators[k].next_floor = elevators[k].elevator_going_to +1; // ap: change this to elevators[k].next_floor
                             ...
                             //   int counter=0;
                               ...
                               Load_Event(&event_head,elevators[k].elevator_clock+elevators[k].elevator_time[0], ELEVATOR_ARRIVAL , next_event.entity_type, next_event.entity_index);  // add ELVEVATOR_ARRIVAL event into the list AP: add this one to revised time for this load event
			...
			if (next_event.event_type == PERSON_ARRIVES_LOBBY)
			{
				...
				if (elevator_avail)  //this means someone arrives to an idle elevator at their floor.
					...
					if ((elevators[elevator_index].num_people == 4)|((people_queue_lobby==0)&(k==1)))
					//if (elevators[elevator_index].num_people == 4)
						{
                        elevators[elevator_index].ultimate_number = elevators[elevator_index].num_people;
						elevators[elevator_index].idle = 0;
						elevators[elevator_index].direction = UP;
                       // int story=20 ;
                       // int counter=0;
						{for (i = 0; i < elevators[elevator_index].ultimate_number; i++)
							...
							elevators[elevator_index].elevator_clock = people[next_event.entity_type][next_event.entity_index].arrive_to_elevator_time; // This is because the idea of time capture in idle situation for elevator
                            ...
                            if (floor_this_person_is_going_to!=story)
                                counter++;
                            story=floor_this_person_is_going_to;
							travel_time = elevator_travel_time_per_floor * (elevators[elevator_index].next_floor-elevators[elevator_index].current_floor) + counter* DOOR_TIME;
							...
							people[next_event.entity_type][next_event.entity_index].total_time_to_get_clinic = people[next_event.entity_type][next_event.entity_index].arrive_to_elevator_time + travel_time - DOOR_TIME/2  ; 
							Load_Event(&event_head,max(people[next_event.entity_type][next_event.entity_index].start_time, people[next_event.entity_type][next_event.entity_index].total_time_to_get_clinic)+people[next_event.entity_type][next_event.entity_index].appointment, CLINIC_DEPARTURE , next_event.entity_type, next_event.entity_index);
                            elevators[elevator_index].num_people -- ;
							//if (k==1) // if this is last person then load event as 
							if (((i == elevators[elevator_index].ultimate_number-1)&(elevators[elevator_index].elevator_going_to==0))|((i == elevators[elevator_index].ultimate_number-1)&(elevators[elevator_index].elevator_going_to<people[next_event.entity_type][next_event.entity_index].to_floor)))

							{
							elevators[elevator_index].elevator_time[0] += travel_time + counter * DOOR_TIME;
							Load_Event(&event_head,elevators[elevator_index].elevator_clock+elevators[elevator_index].elevator_time[0], ELEVATOR_ARRIVAL , next_event.entity_type, next_event.entity_index);  // add ELVEVATOR_ARRIVAL event into the list AP: add this one to revised time for this load event
                            int story=20 ;
                            int counter=0;
			...
			elevators[elevator_index].next_floor=max(0,elevators[elevator_index].next_floor-1);
            elevators[elevator_index].elevator_time[1] += elevator_travel_time_per_floor;
            last_indicator = 0;
			{while ((num_in_line_from_clinic[elevators[elevator_index].next_floor]!=0) & (elevators[elevator_index].num_people<4))
					{next_in_Line = hall_head[DOWN][elevators[elevator_index].next_floor]->Person;
                    if (story != elevators[elevator_index].next_floor)
                        {counter ++;}
                    /// ap: calculate wait time for a clinic departure
                    story = elevators[elevator_index].next_floor;
                    people[next_in_Line.person_type][next_in_Line.index].elevator_wait_time_down=(elevators[elevator_index].elevator_time[1]+ counter * DOOR_TIME + elevators[elevator_index].elevator_time[0]+elevators[elevator_index].elevator_clock)-people[next_in_Line.person_type][next_in_Line.index].total_time_to_get_clinic+people[next_in_Line.person_type][next_in_Line.index].appointment;
					Remove_Event_Person(&hall_head[DOWN][elevators[elevator_index].next_floor]);
					num_in_line_from_clinic[elevators[elevator_index].next_floor] -- ;
					Load_Event_Elevator(&elevator_head[elevator_index],next_event.time+elevator_travel_time_per_floor, next_in_Line.person_type, next_in_Line.index,LOBBY);
                    elevators[elevator_index].num_people ++;
                    last_indicator = 1;
			...
			if (elevators[elevator_index].next_floor != LOBBY )
					{Load_Event(&event_head, next_event.time+elevator_travel_time_per_floor + DOOR_TIME*last_indicator , ELEVATOR_ARRIVAL , next_event.entity_type, next_event.entity_index);
                    last_indicator = 0;
					}
			if ((elevators[elevator_index].num_people==4) & (num_in_line_from_clinic[elevators[elevator_index].next_floor]!=0))
				{for (i=0;i<num_in_line_from_clinic[elevators[elevator_index].next_floor];i++)
					{next_in_Line = hall_head[DOWN][elevators[elevator_index].next_floor]->Person;
					Remove_Event_Person(&hall_head[DOWN][elevators[elevator_index].next_floor]);
					num_in_line_from_clinic[elevators[elevator_index].next_floor] -- ;
					Load_Event(&event_head, next_event.time+elevator_travel_time_per_floor+ EPSILON + DOOR_TIME , CLINIC_DEPARTURE , next_in_Line.person_type, next_in_Line.index);//hp: door_time
					}
				}
            if (elevators[elevator_index].next_floor == LOBBY )
                {   elevators[elevator_index].elevator_time[1] += counter * DOOR_TIME;
                    story = 20;
                    counter = 0;
                    for (i=0;i<elevators[elevator_index].num_people;i++)
                        {Remove_Event_Elevator(&elevator_head[elevator_index]);
                        }
                    // new version of elevator arrival without send_elevator function START HERE 
	                elevators[elevator_index].current_floor = 0;
	...
	// we are going to check if we have a person waiting in a queue as we have a elevator in lobby in idle condition
	                while ((people_queue_lobby!=0)&(elevators[elevator_index].num_people<ELEVATOR_CAPACITY))
		...
		// start from here !!
                        ...
                        floor_this_person_is_going_to = people[next_event.entity_type][next_event.entity_index].to_floor;
                        Load_Event_Elevator(&elevator_head[elevator_index],next_event.time, next_in_Line.time , next_event.entity_index,people[next_event.entity_type][next_event.entity_index].to_floor); // add person to the elevator list
		                }
	                 ...
	                 if (elevators[elevator_index].num_people!=0)
	                    {elevators[elevator_index].ultimate_number = elevators[elevator_index].num_people;
                        for (int i = 0; i < elevators[elevator_index].ultimate_number; i++)
                            {next_in_elevator = elevator_head[elevator_index]->Person;
                            Remove_Event_Elevator(&elevator_head[elevator_index]);
                            next_event.entity_type=next_in_elevator.person_type;
                            next_event.entity_index=next_in_elevator.index;
                            ...
                            //elevators[elevator_index].elevator_tot_time = elevators[elevator_index].elevator_time[0] + elevators[elevator_index].elevator_time[1];
							...
							if ((i == elevators[elevator_index].ultimate_number-1)&(elevators[elevator_index].elevator_going_to>people[next_event.entity_type][next_event.entity_index].to_floor))
                                ...
                                int story=20 ;
                                int counter=0; // add ELVEVATOR_ARRIVAL event into the list
                                } // add ELVEVATOR_ARRIVAL event into the list
                // new version of elevator arrival without send_elevator function END HERE 
				            }
				        }
			        }
                }
	...
	if (elevators[index].num_people!=0)
	{
		for (int i = 0; i < elevators[index].num_people; i++)
			...
			if (i == elevators[index].num_people-1) // this is last person in the elevator
				...
				{
				//Load_Event(&event_head, elevators[index].elevator_time[0], ELEVATOR_ARRIVAL , next_event.entity_type, next_event.entity_index);  // add ELVEVATOR_ARRIVAL event into the list
				Load_Event(&event_head,elevators[index].elevator_clock+travel_time, ELEVATOR_ARRIVAL , next_event.entity_type, next_event.entity_index);  // add ELVEVATOR_ARRIVAL event into the list
				}
				}
			}
