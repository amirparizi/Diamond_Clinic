    ...
    keepgoing=1;
		...
		//for (ii=0;ii<0;ii++)
		{
				...
				{	
					...
					for (i = 0; i < NUM_ELEVATORS; i++) 
                        ...
                        if ((elevators[i].next_floor<people[next_event.entity_type][next_event.entity_index].to_floor)&(elevators[i].idle==0))
						    {travel_time = elevator_travel_time_per_floor * (people[next_event.entity_type][next_event.entity_index].to_floor+elevators[i].next_floor);}//hP we chinged this from - to +
						...
						elevators[i].time_to_reach = elevators[i].elevator_clock + travel_time - next_event.time;}
					}
					...
					{ if (wait_time == elevators[k].time_to_reach)
						...
						if (elevators[k].next_floor < people[next_event.entity_type][next_event.entity_index].to_floor)
                            ...
                            if (elevators[k].idle==1)
                               ...
                               elevators[k].elevator_time[0] += travel_time;
                               elevators[k].idle=0;
				...
				if (elevator_avail)  //this means someone arrives to an idle elevator at their floor.
					...
					//if (elevators[elevator_index].num_people == 4)
						...
						{for (i = 0; i < elevators[elevator_index].ultimate_number; i++)
                            ...
                            people[next_event.entity_type][next_event.entity_index].elevator_travel_time = travel_time;
							people[next_event.entity_type][next_event.entity_index].total_time_to_get_clinic = people[next_event.entity_type][next_event.entity_index].arrive_to_elevator_time + travel_time  ; 
							...
							{
							elevators[elevator_index].elevator_time[0] += travel_time;
			...
			{while ((num_in_line_from_clinic[elevators[elevator_index].next_floor]!=0) & (elevators[elevator_index].num_people<4))
                    ...
                    people[next_in_Line.person_type][next_in_Line.index].elevator_wait_time_down=(elevators[elevator_index].elevator_time[1]+elevators[elevator_index].elevator_time[0]+elevators[elevator_index].elevator_clock)-people[next_in_Line.person_type][next_in_Line.index].total_time_to_get_clinic+people[next_in_Line.person_type][next_in_Line.index].appointment;
			...
			if (elevators[elevator_index].next_floor != LOBBY )
					{Load_Event(&event_head, next_event.time+elevator_travel_time_per_floor, ELEVATOR_ARRIVAL , next_event.entity_type, next_event.entity_index); //hp: door_time
					}
			if ((elevators[elevator_index].num_people==4) & (num_in_line_from_clinic[elevators[elevator_index].next_floor]!=0))
				{for (i=0;i<num_in_line_from_clinic[elevators[elevator_index].next_floor];i++)
					...
					Remove_Event_Person(&hall_head[DOWN][elevators[elevator_index].next_floor]);
					num_in_line_from_clinic[elevators[elevator_index].next_floor] -- ;
					Load_Event(&event_head, next_event.time+elevator_travel_time_per_floor+EPSILON, CLINIC_DEPARTURE , next_in_Line.person_type, next_in_Line.index);//hp: door_time
					}
				}
            if (elevators[elevator_index].next_floor == LOBBY )
		...
		// start from here !!
	                 ...
	                 if (elevators[elevator_index].num_people!=0)
                        ...
                        for (int i = 0; i < elevators[elevator_index].ultimate_number; i++)
							...
							people[next_event.entity_type][next_event.entity_index].total_time_to_get_clinic = people[next_event.entity_type][next_event.entity_index].arrive_to_elevator_time + travel_time + people[next_event.entity_type][next_event.entity_index].elevator_wait_time; 
							Load_Event(&event_head,people[next_event.entity_type][next_event.entity_index].total_time_to_get_clinic+people[next_event.entity_type][next_event.entity_index].appointment, CLINIC_DEPARTURE , next_event.entity_type, next_event.entity_index);
                            elevators[elevator_index].num_people -- ;
							if (((i == elevators[elevator_index].ultimate_number-1)&(elevators[elevator_index].elevator_going_to==0))|((i == elevators[elevator_index].ultimate_number-1)&(elevators[elevator_index].elevator_going_to<people[next_event.entity_type][next_event.entity_index].to_floor)))
